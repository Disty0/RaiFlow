...
def SoteDiffusionV3PosEmbed1D(embeds: torch.Tensor, sigmas: torch.Tensor, latents_seq_len: int):
    ...
    posed_embeds[:, :, 2] = posed_embeds[:, :, 2] * sigmas.expand(batch_size, seq_len)
    posed_embeds[:, :, 3] = posed_embeds[:, :, 3] - sigmas.expand(batch_size, seq_len) # add this to make the dim lenght even
...
def SoteDiffusionV3PatchEmbed2D(latents: torch.Tensor, sigmas: torch.Tensor, patch_size: int, embeds_seq_len: int):
